...
public class InstallationRepositoryITest {
    ...
    private void buildAndPersistInstallations() {
        ...
        final Installation installation1 = prebuildInstallation(Supplier.LUFTDATEN, SUPPLIER_INSTALLATION_ID_1).build();
        final Installation installation2 = prebuildInstallation(Supplier.LUFTDATEN, SUPPLIER_INSTALLATION_ID_2).build();
        final Installation installation3 = prebuildInstallation(Supplier.GIOS, SUPPLIER_INSTALLATION_ID_3).build();
        List<Installation> persistedInstallations = repository.saveAll(asList(installation1, installation2, installation3));

        // add new received measurements to existing installations
        final Measurement measurement1 = prebuildMeasurement(DATE_1).installation(persistedInstallations.get(0)).build();
        final Measurement measurement2 = prebuildMeasurement(DATE_2).installation(persistedInstallations.get(1)).build();
        final Measurement measurement3 = prebuildMeasurement(DATE_3).installation(persistedInstallations.get(1)).build();
        final Measurement measurement4 = prebuildMeasurement(DATE_4).installation(persistedInstallations.get(2)).build();
        ...
        List<Installation> installations = measurements.stream().map(Measurement::getInstallation).collect(toList());
    ...
    public void findSelectedProviderRelatedData() {
        ...
        // When
        Optional<Installation> result = repository.findByProvider(Supplier.LUFTDATEN, SUPPLIER_INSTALLATION_ID_2);
